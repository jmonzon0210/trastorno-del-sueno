...
import { Dropdown, Select, Button, Modal, Popconfirm, Input, Space, Form, message, Tag, Menu} from "antd";
import { SearchOutlined, MoreOutlined, ReloadOutlined, FilePdfFilled, FileExcelFilled, CheckCircleTwoTone } from "@ant-design/icons";
...
export default function Historial() {
 ...
 <Menu>
    <Menu.Item key="editar" onClick={() => handleEdit(record)}>
      <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
        <ModeEditIcon style={{ fontSize: 18, color: "#1890ff" }} />
        Editar
      </span>
    </Menu.Item>
    <Menu.Item key="diagnostico" onClick={() => handleDiagnostico(record)}>
      <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
  ...
  const columns = [
    ...
    { title: () => <div>Carnet de<br />Identidad</div>, dataIndex: "carnet_identidad", key: "carnet_identidad" },
  ...
  const exportToCSV = () => {
    const headers = [
      "ID", "Resultado", "Confirmado", "Nombre", "Carnet", "Sexo", "Edad",
      "AF", "APPPP", "AA",
      "CM", "CT", "EMP",
      "TND", "OB", "HA",
      "TA", "TC", "CEF",
  ...
  const exportToPDF = () => {
    ...
    doc.text("Historial de Pacientes", 20, 10); 
     // Generar la tabla en el PDF
     const headers = [
      "ID", "Resultado", "Verificado", "Nombre", "CI", "Sexo", "Edad"
      
    ];
    const body = filteredData.map((row) => [
      row.id, row.resultado,     row.diagnostico_real !== null ? "Sí" : "",
      row.nombre_completo, row.carnet_identidad,row.sexo, row.edad
    ]);
